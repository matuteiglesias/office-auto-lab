...
def _recipe_paths() -> set[str]:
    ...
    return set(re.findall(r"(?:bash|python3)\s+((?:src|scripts)/[^\s]+\.(?:py|sh))", text))
...
def test_all_file_entrypoints_referenced_by_make_exist() -> None:
    paths = _recipe_paths()
    assert paths
    assert sorted(path for path in paths if not (REPO_ROOT / path).is_file()) == []
...
def test_smoke_is_core_only_and_legacy_compiler_is_compatibility_only() -> None:
    ...
    assert smoke_deps == [
        "imports", "control-contracts", "identity-contracts", "work-contracts",
        "staff-v2-contracts", "editorial-contracts", "runtime-contracts", "repo-scans",
    ]
    ...
    assert "\ncontrol-contracts:" in text and "tests.test_control_snapshot_v2" in text
    assert "\nidentity-contracts:" in text and "tests.test_identity_resolution_v2" in text
    assert "\nwork-contracts:" in text and "tests.test_work_item_compiler_v1" in text
    assert "\nstaff-v2-contracts:" in text and "tests.test_staff_preparation_v2" in text
