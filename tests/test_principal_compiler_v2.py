...
def packet(
    ...
    kind: str,
    *,
    principal_needed: bool = False,
    status: str = "PREPARED_DEEP",
    blockers: list[str] | None = None,
    needs: str = "current bounded need",
    ...
    return {
        ...
        "current_state": {
            ...
            "principal_mode": "REQUIRED" if principal_needed else "RECOMMENDED",
            "needs": needs,
            "note": "context note",
            "runtime": {"health_status": "OK", "health_bucket": "TEST"},
        },
        "identity": {
        ...
        "evidence": [{
            "adapter": "local_repo",
            "status": "ok",
            "repo_id": f"repo.{front_id}",
            "workspace_id": f"ws.{front_id}",
            "revision": "abc123",
            "dirty": False,
...
def preparation(*packets: dict) -> dict:
    return {
        ...
        "source_snapshot_digest": "sha256:snapshot",
        "preparation_digest": "sha256:prep",
...
class PrincipalCompilerV2Tests(unittest.TestCase):
    ...
    def test_principal_item_reaches_needs_you_only_after_successful_preparation(self) -> None:
        ...
        blocked = compile_principal_brief(preparation(packet(
            "fr_decide", "DECIDE", principal_needed=True, status="BLOCKED", blockers=["missing evidence"]
        )))
    ...
    def test_delta_reports_added_cleared_and_changed_entries(self) -> None:
        ...
        current = compile_principal_brief(
            preparation(
