...
class OfficeIdentityRepoContextTests(unittest.TestCase):
    ...
    def test_pandas_na_repo_ids_are_empty(self):
        df = pd.DataFrame([{"front_id": "front-a", "repo_ids": pd.NA}])
        out, summary = enrich_with_repo_context(df, None)
        self.assertEqual(out.loc[0, "repo_context_status"], "none")
        self.assertEqual(summary["repo_refs"], 0)
