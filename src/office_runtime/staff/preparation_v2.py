...
def _priority_tuple(item: dict) -> tuple[int, int, int, str]:
    ...
    horizon = str(item.get("horizon", "")).upper()
    principal = 0 if item.get("principal_required") else 1
    return (
        principal,
        KIND_RANK.get(kind, 99),
        HORIZON_RANK.get(horizon, 50),
...
def triage_work_item(work_item: dict) -> TriageResult:
    ...
    workspace_states = identity.get("workspace_states", []) or []
    workspace_statuses = {str(row.get("status", "")).upper() for row in workspace_states}
    reasons: list[str] = []

    if identity_status == "ERROR":
        reasons.append("IDENTITY_ERROR")
        return TriageResult(work_item_id, front_id, kind, "BLOCKED", "LIGHT", tuple(reasons))

    if kind == "UNBLOCK" and (
        identity_status == "NOT_READY"
        or workspace_statuses.intersection({"AMBIGUOUS", "UNRESOLVED", "UNAVAILABLE"})
    ):
    ...
    if kind == "MAINTAIN" and prep_mode == "NONE":
        reasons.append("BOUNDED_MAINTENANCE")
        return TriageResult(work_item_id, front_id, kind, "NO_DEEP_PREP_REQUIRED", "LIGHT", tuple(reasons))

    if kind in DEEP_KINDS or prep_mode == "STAFF_REQUIRED":
        reasons.append("NEAR_PULL_FRONTIER")
    ...
    reasons.append("LIGHT_PREPARATION_ONLY")
    return TriageResult(work_item_id, front_id, kind, "READY_LIGHT", "LIGHT", tuple(reasons))
...
class SnapshotEvidenceAdapter:
    ...
    def collect(self, snapshot: dict, work_item: dict) -> dict:
        ...
        support = [
            row
            for row in _rows(snapshot, "support_artifacts_v2")
            if str(row.get("front_id", "")).strip() == front_id
            and str(row.get("status", "")).strip().upper() not in {"RETIRED", "DEPRECATED"}
        ]
        contracts = [
            row
            ...
            "operator_contracts": [
                {
                    ...
                    for key in (
                        ...
                        "forbidden_powers",
                        "required_seams",
                        "must_consume_shared_modules",
                        "must_not_implement_locally",
                    )
                    if key in row and str(row.get(key, "")).strip()
                }
...
class LocalRepoEvidenceAdapter:
    """Collect bounded local Git evidence through governed workspace identity.

    Concrete paths are used only inside this adapter to inspect the selected
    workspace. Paths are deliberately omitted from the returned portable evidence.
    """
    ...
    def collect(self, snapshot: dict, work_item: dict) -> dict:
        ...
        if workspace.get("status") != "RESOLVED" or not workspace.get("local_path"):
            return {
                ...
                "reason": workspace.get("reason") or "workspace is not resolved",
            }
        ...
        if not path.is_dir():
            return {
                ...
                "repo_id": repo.get("repo_id"),
                "workspace_id": workspace.get("workspace_id"),
                "reason": "resolved workspace path is not present on this machine",
            }
...
def _recommended_move(work_item: dict, blockers: list[str]) -> str:
    ...
    }.get(kind, "Clarify the next bounded move.")
...
def _packet(
    ...
    *,
    ...
    packet = {
        ...
        "principal_question": (
            "Choose or authorize the bounded next move for this item."
            if bool(work_item.get("principal_required")) or str(work_item.get("kind", "")).upper() == "DECIDE"
            else ""
        ),
...
def prepare_work_items(
    ...
    *,
    ...
    The caller supplies one immutable Control Tower snapshot and the work-item set
    compiled from that exact snapshot. Deep preparation is bounded by ``max_deep``;
    all items still receive cheap deterministic triage.
    """
    ...
    for item in items:
        ...
        if run_deep and deep_used >= max_deep:
            packets.append(
                _packet(
                    ...
                    [],
                    prepared_at=prepared_at,
                    preparation_status="DEFERRED_BUDGET",
                )
            )
            continue
        ...
        if triage.status != "BLOCKED":
            ...
            # Blocked items still receive snapshot evidence so the blocker can be
            # presented coherently without invoking expensive/local adapters.
