...
def _target(entry: dict) -> tuple[dict, str | None]:
    ...
    if repo_ids and unresolved:
        ...
        }, "repository target is not fully resolved"
    return {
        "front_id": entry.get("front_id", ""),
        "repo_ids": repo_ids,
        "workspaces": workspace_states,
...
def compile_execution_plan(snapshot: dict, principal_brief: dict) -> dict:
    ...
    be reconciled into governed state (or another explicit approval contract in a
    future version) and surface as executable work on a subsequent Office run.
    """
